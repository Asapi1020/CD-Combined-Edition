...
function DrawMenu()
{
	...
	if(bOpaque) Canvas.SetDrawColor(30, 30, 30, 255);
	else Canvas.SetDrawColor(30, 30, 30, 200);
	Canvas.SetPos(0.f, 0.f);
	Owner.CurrentStyle.DrawRectBox(XPosition, YPosition, CompPos[2], CompPos[3], int(EdgeSize), 0);
	...
	for (i=0; (i < ListItemsPerPage && C != None); ++i)
	{
		...
		{
			Canvas.SetPos(EdgeSize, Y);
			Canvas.SetDrawColor(250, 100, 0, 200); //Canvas.DrawColor = FocusedLineColor;
			Owner.CurrentStyle.DrawWhiteBox(CompPos[2]-(EdgeSize*2), ItemHeight);
			Canvas.SetDrawColor(250, 250, 250, 255);
		}
