...
var localized string ZedSpawnedString;
var localized string SingleCohortString;
var localized string SpawnRateString;
var localized string CohortString;
var localized string SquadString;
...
//	local string DelayString, SpawnDurationString, LingerString;
	...
	{
		return "0/" $ WaveTotalAI $ ZedSpawnedString;
	}
...
static function string FormatFloatToTwoDecimalPlaces( const float f )
{
	local int l;
	local string s;	
...
function SetupNextWave(byte NextWaveIndex, int TimeToNextWaveBuffer = 0)
{
	...
	if (MyKFGRI.IsBossWave() && BossEnum == CDBOSS_ALL){
		WaveTotalAI = 5;
	}
}
...
function GetSpawnListFromSquad(byte SquadIdx, out array< KFAISpawnSquad > SquadsList, out array< class<KFPawn_Monster> >  AISpawnList)
{
		...
		for ( j = 0; j < SquadElements[i].Num; j++ )
		{
			...
			else
			{
				...
				if( AIType == AT_BossRandom )
				{
					...
					else if ( BossEnum == CDBOSS_ALL )
					{
						`cdlog("Spawning All (config: Boss="$Outer.BossEnum$")", bLogControlledDifficulty);
						TempSpawnList.AddItem(AIBossClassList[BAT_Hans]);
						TempSpawnList.AddItem(AIBossClassList[BAT_Patriarch]);
						TempSpawnList.AddItem(AIBossClassList[BAT_KingFleshpound]);
						TempSpawnList.AddItem(AIBossClassList[BAT_KingBloat]);
					...
					else if ( BossEnum == CDBOSS_CYST )
					{
						`cdlog("Spawning Cyst (config: Boss="$Outer.BossEnum$")", bLogControlledDifficulty);
						TempSpawnList.AddItem(class'CD_Pawn_ZedCystKing');
					}
					else
					{
	...
	if ( !AlbinoCrawlersBool )
	{
		...
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedCrawler_Regular',
		                 AISpawnList );
	}
	...
	if ( !AlbinoAlphasBool )
	{
		...
		MatchClasses[1] = class'CombinedCD2.CD_Pawn_ZedClot_Alpha_Special';
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedClot_Alpha_Regular',
		                 AISpawnList );
	}
	else
	{
		...
		MatchClasses[0] = AIClassList[AT_AlphaClot];
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedClot_Alpha',
		                 AISpawnList );
	}
	...
	if ( !AlbinoGorefastsBool )
	{
		...
		MatchClasses[1] = class'CombinedCD2.CD_Pawn_ZedGorefast_Special';
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedGorefast_Regular',
		                 AISpawnList );
	}

	if ( bDisableRobots )
	{
		`cdlog("DisableRobots="$bDisableRobots$": scanning AISpawnList of length "$AISpawnList.Length$" at squadidx "$SquadIdx, bLogControlledDifficulty);
		...
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedStalker_Regular',
		                 AISpawnList );
	}
	...
	if ( bDisableRobots )
	{
		...
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedHusk_Regular',
		                 AISpawnList );
	}
	...
	if ( bDisableRobots )
	{
		...
		MatchClasses[0] = AIClassList[AT_EDAR_EMP];
		MatchClasses[1] = AIClassList[AT_EDAR_Laser];
		MatchClasses[2] = AIClassList[AT_EDAR_Rocket];
		ReplaceZedClass( MatchClasses,
		                 (FRand() < 0.5f) ? class'CombinedCD2.CD_Pawn_ZedHusk_Regular' : class'CombinedCD2.CD_Pawn_ZedStalker_Regular',
		                 AISpawnList );
	}
	...
	if ( bLargeLess )
	{
		...
		MatchClasses[3] = class'KFGameContent.KFPawn_ZedFleshpoundMini';
		MatchClasses[4] = class'CD_Pawn_ZedFleshpoundMini_RS';
		MatchClasses[5] = class'CD_Pawn_ZedFleshpoundMini_NRS';
		MatchClasses[6] = class'KFGameContent.KFPawn_ZedScrake';
		ReplaceZedClass( MatchClasses,
	...
	if ( !FleshpoundRageSpawnsBool )
	{
		...
		MatchClasses[1] = class'CombinedCD2.CD_Pawn_ZedFleshpound_RS';
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedFleshpound_NRS',
		                 AISpawnList );
		...
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedFleshpoundMini_NRS',
		                 AISpawnList );
	}

	if ( bDisableBossMinions )
	{
		`cdlog("DisableBossMinions="$bDisableBossMinions$": scanning AISpawnList of length "$AISpawnList.Length$" at squadidx "$SquadIdx, bLogControlledDifficulty);

		MatchClasses.Length = 1;
		MatchClasses[0] = AIBossClassList[BAT_KingBloat];
		ReplaceZedClass( MatchClasses,
		                 class'CombinedCD2.CD_Pawn_ZedBloatKing',
		                 AISpawnList );
	}
}
